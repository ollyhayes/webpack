...
function endsWith(str, end) {
	...
	for(var i = str.length - 1; i > 0; i--) {
		...
		if(cc === end) return true;
		if(SPACINGS.indexOf(cc) >= 0) continue;
		return false;
	}
	return false;
}
...
NormalModule.prototype.doBuild = function doBuild(options, compilation, resolver, fs, callback) {
	...
	runLoaders({
		...
		module._json = false;
		// guess if the module is JSON (for performance)
		if((/^\s*\{/.test(source) && endsWith(source, "}")) ||
			(/^\s*\[/.test(source) && endsWith(source, "]")) ||
			(/^\s*"/.test(source) && endsWith(source, "\"")) ||
			(/^\s*'/.test(source) && endsWith(source, "'")) ||
			/^\s*(true|false|null|-?(0|[1-9][0-9]*)(\.[0-9]+)?([eE][+\-]?[0-9]+)?)\s*$/.test(source)) {
			var isJSON;
			try {
				JSON.parse(source);
				isJSON = true;
			...
			if(isJSON) {
				...
				if(module.identifier) {
					module._source = new OriginalSource("module.exports = " + source, module.identifier());
				} else {
					module._source = new RawSource("module.exports = " + source);
				}
				return callback();
			}
		}
...
NormalModule.prototype.build = function build(options, compilation, resolver, fs, callback) {
	...
	return _this.doBuild(options, compilation, resolver, fs, function(err) {
		...
		if(_this._json) return callback();
